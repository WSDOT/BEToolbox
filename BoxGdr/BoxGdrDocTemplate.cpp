...
: CEAFDocTemplate(nIDResource,pCallback,pDocClass,pFrameClass,pViewClass,hSharedMenu,maxViewCount)
{
   ...
   HICON hIcon = AfxGetApp()->LoadIcon(IDR_BOXGDR);
...
CString CBoxGdrDocTemplate::GetTemplateGroupItemDescription(const CEAFTemplateItem* pItem) const
{
   CString strDescription(_T("Create a new BoxGdr project (Box Girder Section Properties). "));
