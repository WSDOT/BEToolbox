...
IMPLEMENT_DYNAMIC(CCurvelDocTemplate,CEAFDocTemplate)

CCurvelDocTemplate::CCurvelDocTemplate(UINT nIDResource,
...
CString CCurvelDocTemplate::GetTemplateGroupItemDescription(const CEAFTemplateItem* pItem) const
{
   CString strDescription(_T("Create a new Curvel project (Bridge Elevations)."));
...
CDocument* CCurvelDocTemplate::OpenDocumentFile(LPCTSTR lpszPathName,BOOL bMakeVisible)
{
   ...
   if ( lpszPathName == NULL )
   {
      ...
      CCurvelRptView* pRptView = (CCurvelRptView*)pActiveView;

      pRptView->NotifyReportButtonWasClicked();
   }
